...
Count `added` and `modified` entries as changed. Exclude `unchanged` context entries.
...
Let `N` be the number of methods listed in changed classes. These are the added or modified public methods required by the Architecture Diff.

Score: `N`. Lower is better. The minimum score is 0.

### Changed Dataflow Relationships

Let `N` be the number of `dataflow` relationships whose `changeType` is `added` or `modified`.
