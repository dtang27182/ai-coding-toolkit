...
- Represent the Implementation Dataflow narrative, including all changes required by the User Flow Steps. Keep broader workflows in Relevant Logic and Dataflow, outside the diagram.
...
- Give every class a `stateVariables` array. Include every mutable instance variable added, modified, or deleted by the design and every unchanged mutable instance variable read or written by the Implementation Dataflow narrative. Use `[]` when none qualify.
...
- Add state relationships for all state variables read or written by the Implementation Dataflow narrative, including cross-class access.
