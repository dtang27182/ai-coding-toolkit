...
export function routeCompositionEdge(from: Rect, to: Rect, obstacles: Rect[]): string {
  ...
  let path: string;
  if (points.slice(1).some((point, index) => segmentBlocked(points[index], point, blocked))) {
    path = polylinePath(detour(start, end, 1, -1, blocked, 20));
  } else {
...
function curveBlocked(points: Point[], obstacles: Rect[], depth = 0): boolean {
  // Subdivision bounds the entire curve, so even a narrow obstacle cannot be skipped between samples.
...
function detour(start: Point, end: Point, departure: number, arrival: number, obstacles: Rect[], clearance: number): Point[] {
  // Search horizontal/vertical corridors along obstacle boundaries, with vertical endpoint connections.
  ...
  // Each point has a horizontal and vertical arrival state, so bends can be penalized.
...
function curvedPath(points: Point[], departure: number, arrival: number, obstacles: Rect[], clearance: number): string {
  ...
  // Remove unnecessary waypoints only when the resulting broad curves clear every obstacle.
  ...
  for (let index = 0; index < points.length - 1; index += 1) {
    ...
    while (true) {
      curve = [from,
        ...
        { x: to.x - tangents[index + 1].x * handle, y: to.y - tangents[index + 1].y * handle }, to];
...
function polylinePath(points: Point[]): string {
  return pathTurns(points).map((point, index) => `${index === 0 ? "M" : "L"} ${point.x} ${point.y}`).join(" ");
}
